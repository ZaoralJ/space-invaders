...
function createSoundEngine() {
  ...
  let marchStep = 0;
  // Classic march pitches (Hz): four descending tones
  ...
  // Generic envelope: osc + optional noise into gain node
  ...
  function playShoot() {
    resume();
    playTone({ startFreq: 900, endFreq: 400, type: 'square', duration: 0.1, volume: 0.25 });
  }

  function playAlienKilled() {
    resume();
    // Noise burst + pitched blip
    playTone({ noise: true, duration: 0.18, volume: 0.4 });
    playTone({ startFreq: 600, endFreq: 80, type: 'sawtooth', duration: 0.2, volume: 0.2 });
  }

  function playPlayerHit() {
    resume();
    playTone({ noise: true, duration: 0.5, volume: 0.5 });
    playTone({ startFreq: 200, endFreq: 40, type: 'sawtooth', duration: 0.5, volume: 0.3 });
  }

  function playUFOKilled() {
    resume();
    playTone({ noise: true, duration: 0.3, volume: 0.5 });
    // Three quick descending beeps
    [400, 300, 180].forEach((f, i) => {
      const c2 = getCtx();
      const g = c2.createGain();
      g.gain.setValueAtTime(0.25, c2.currentTime + i * 0.07);
  ...
  function playMarch() {
    ...
    marchStep++;
    playTone({ freq, type: 'square', duration: 0.06, volume: 0.2 });
  }
  ...
  function startUFO() {
    ...
    ufoGain.gain.setValueAtTime(0.15, c2.currentTime);
    ...
    // Oscillate between two pitches for that classic UFO warble
  ...
  function stopUFO() {
    if (ufoOsc) {
      try {
        if (ufoOsc._lfo) ufoOsc._lfo.stop();
        ufoOsc.stop();
      } catch (_) {}
  ...
  function playGameOver() {
    ...
    [[300, 0], [250, 0.3], [200, 0.6], [150, 0.9], [100, 1.2]].forEach(([f, t]) => {
      const c2 = getCtx();
      const g = c2.createGain();
      g.gain.setValueAtTime(0.3, c2.currentTime + t);
  ...
  function playWin() {
    ...
    [[262, 0], [330, 0.15], [392, 0.3], [523, 0.45], [659, 0.65], [784, 0.85]].forEach(([f, t]) => {
      const c2 = getCtx();
      const g = c2.createGain();
      g.gain.setValueAtTime(0.25, c2.currentTime + t);
  ...
  let muted = false;
  function setMuted(val) { muted = val; if (val) stopUFO(); }
  function isMuted() { return muted; }

  // Wrap all play* so they no-op when muted
